...
if __name__ == '__main__':
    ...
    while True:
        ...
        if num_now<num_space:
            gpu_id = q.get()
            p = Process(target=agent, args=(q, gpu_id, sweep_id, wandb_config, params_default, agent_package, record_split))
            p.start()
        else:
    ...
    print("record mean: ", np.array(list(record_split)).mean())
    print("standard deviation: ", np.array(list(record_split)).std())