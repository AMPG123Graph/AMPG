...
def runner(wandb_config, params_default, record_split):
    tmpdirname = "remote"
    ...
    try:
        ...
        writer.close()
        record_split.append(best_test_metric)
...
def agent(q, gpu_id, sweep_id, wandb_config, params_default, agent_package, record_split):
    params_default['gpu_index'] = gpu_id
    print('Agent started with GPU [%s]'%(gpu_id))
    try:
        entity,project = os.getenv('WANDB_entity'),os.getenv('WANDB_project')
        wandb.agent(sweep_id, function=lambda:runner(wandb_config, params_default,record_split), entity=entity, project=project, count=agent_package)
    except:
        time.sleep(10)
